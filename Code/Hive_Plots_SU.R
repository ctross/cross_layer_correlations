...
#######################
Give = model_dat_su$Give
Exploit = model_dat_su$Exploit
Reduce = model_dat_su$Reduce
Selfish = model_dat_su$Selfish
Generous = model_dat_su$Generous
...
V(graph)$Standing <- as.character(ifelse(Standing>9,"High",ifelse(Standing < -2, "Low","Average")))
...
V(graph)$site <- rep("Altiplano",length(Standing))
...
ggsave("Punish_SU_Hive.png", P1, height=6, width=8)
ggsave("Give_SU_Hive.png", P3, height=6, width=8.5)
ggsave("Exploit_SU_Hive.png", P2, height=6, width=8)        
