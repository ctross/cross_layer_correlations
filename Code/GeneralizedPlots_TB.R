#################################################### GR
rs_m = c(precis(res_tb,pars="G_corr",depth=3)[,1])
rs_l = c(precis(res_tb,pars="G_corr",depth=3)[,3])
rs_h = c(precis(res_tb,pars="G_corr",depth=3)[,4])
...
r_if_sig = ifelse(precis(res_tb, pars="G_corr",depth=3)[,3] > 0 | precis(res_tb, pars="G_corr",depth=3)[,4] < 0, round(rs_m,2),NA)

df_tb = data.frame(r=rs_m, l=rs_l, h=rs_h, measure1=measure1, measure2=measure2, r_if_sig=r_if_sig, rs_type=rs_type, measure1B=measure1B, measure2B=measure2B)



df_tb$Group1 = ifelse(substrRight(df_tb$measure1,4)=="der)" & substrRight(df_tb$measure2,4)=="der)", "Sender",
                   ifelse(substrRight(df_tb$measure1,4)=="ver)" & substrRight(df_tb$measure2,4)=="ver)", "Receiver",
                   "Cross"
                   ))

df_tb$Group2 = paste0(substrLeft(df_tb$measure1B,9),"_",substrLeft(df_tb$measure2B,9))
df_tb$Group3 = substrLeft(df_tb$measure1B,9)

calming_como = c("#34261D", "#A96922", "#7D370D", "#1C7262","#114B47","#CF821B","#E4A835")

 p1 = ggplot(df_tb, aes(measure1, measure2, fill=r, label=round(r_if_sig,2),color=rs_type)) +
...
ggsave("GR_TB.pdf", p1, height=0.6*12, width=12)


############### Prep for merged plots
df2 = df_tb[which(df_tb$Group1 %in% c("Sender")),]
df2_tb_a = df2[which(!is.na(df2$r)),]

df2 = df_tb[which(df_tb$Group1 %in% c("Receiver")),]
df2_tb_b = df2[which(!is.na(df2$r)),]
