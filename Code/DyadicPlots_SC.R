########################################## DR
rs_m = c(precis(res_sc,pars="D_corr",depth=3)[,1])
rs_l = c(precis(res_sc,pars="D_corr",depth=3)[,3])
rs_h = c(precis(res_sc,pars="D_corr",depth=3)[,4])
...
r_if_sig = ifelse(precis(res_sc,pars="D_corr",depth=3)[,3] > 0 | precis(res_sc,pars="D_corr",depth=3)[,4] < 0, round(rs,2),NA)

df_sc = data.frame(r=c(rs_m), l=c(rs_l), h=c(rs_h), measure1=measure1b, measure2=measure2b, r_if_sig=r_if_sig, rs_type=rs_type)

calming_como = c("#34261D", "#A96922", "#7D370D", "#1C7262","#114B47","#CF821B","#E4A835")

p1 =  ggplot(df_sc, aes(measure1, measure2, fill=r, color=rs_type, label=round(r_if_sig,2))) +
 geom_tile(aes(width=0.96, height=0.96), size=1.5) +
 labs(x = NULL, y = NULL, fill = "Correlation", title="Dyadic reciprocity estimates: Lowland site", subtitle="Only reliable correlation coefficients shown") + 
...
ggsave("DR_SC.pdf", p1, height=6, width=10)
...
df_q_sc = data.frame(r=c(rs_m), l=c(rs_l), h=c(rs_h), measure1=measure1, measure2=measure2, r_if_sig=r_if_sig, rs_type=rs_type)

df_q_sc$Group2 = paste0(substrLeft(df_q_sc$measure2,1),"_",substrLeft(df_q_sc$measure1,1))
df_q_sc$Group3 = substrLeft(df_q_sc$measure1,5)
