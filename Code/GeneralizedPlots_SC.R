#################################################### GR
rs_m = c(precis(res_sc,pars="G_corr",depth=3)[,1])
rs_l = c(precis(res_sc,pars="G_corr",depth=3)[,3])
rs_h = c(precis(res_sc,pars="G_corr",depth=3)[,4])
...
r_if_sig = ifelse(precis(res_sc, pars="G_corr",depth=3)[,3] > 0 | precis(res_sc, pars="G_corr",depth=3)[,4] < 0, round(rs_m,2),NA)

df_sc = data.frame(r=rs_m, l=rs_l, h=rs_h, measure1=measure1, measure2=measure2, r_if_sig=r_if_sig, rs_type=rs_type, measure1B=measure1B, measure2B=measure2B)



df_sc$Group1 = ifelse(substrRight(df_sc$measure1,4)=="der)" & substrRight(df_sc$measure2,4)=="der)", "Sender",
                   ifelse(substrRight(df_sc$measure1,4)=="ver)" & substrRight(df_sc$measure2,4)=="ver)", "Receiver",
                   "Cross"
                   ))

df_sc$Group2 = paste0(substrLeft(df_sc$measure1B,9),"_",substrLeft(df_sc$measure2B,9))
df_sc$Group3 = substrLeft(df_sc$measure1B,9)

calming_como = c("#34261D", "#A96922", "#7D370D", "#1C7262","#114B47","#CF821B","#E4A835")

 p1 = ggplot(df_sc, aes(measure1, measure2, fill=r, label=round(r_if_sig,2),color=rs_type)) +
 geom_tile(aes(width=0.96, height=0.96), size=1.69) +
 labs(x = NULL, y = NULL, fill = "Correlation", title="Generalized reciprocity estimates: Lowland site", subtitle="Only reliable correlation coefficients shown") + 
...
ggsave("GR_SC.pdf", p1, height=0.6*12, width=12)


############### Prep for merged plots
df2 = df_sc[which(df_sc$Group1 %in% c("Sender")),]
df2_sc_a = df2[which(!is.na(df2$r)),]

df2 = df_sc[which(df_sc$Group1 %in% c("Receiver")),]
df2_sc_b = df2[which(!is.na(df2$r)),]
