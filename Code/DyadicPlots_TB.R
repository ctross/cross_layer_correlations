########################################## DR
rs_m = c(precis(res_tb,pars="D_corr",depth=3)[,1])
rs_l = c(precis(res_tb,pars="D_corr",depth=3)[,3])
rs_h = c(precis(res_tb,pars="D_corr",depth=3)[,4])
...
r_if_sig = ifelse(precis(res_tb,pars="D_corr",depth=3)[,3] > 0 | precis(res_tb,pars="D_corr",depth=3)[,4] < 0, round(rs,2),NA)

df_tb = data.frame(r=c(rs_m), l=c(rs_l), h=c(rs_h), measure1=measure1b, measure2=measure2b, r_if_sig=r_if_sig, rs_type=rs_type)

calming_como = c("#34261D", "#A96922", "#7D370D", "#1C7262","#114B47","#CF821B","#E4A835")

p1 =  ggplot(df_tb, aes(measure1, measure2, fill=r, color=rs_type, label=round(r_if_sig,2))) +
...
ggsave("DR_TB.pdf", p1, height=6, width=10)
...
df_q_tb = data.frame(r=c(rs_m), l=c(rs_l), h=c(rs_h), measure1=measure1, measure2=measure2, r_if_sig=r_if_sig, rs_type=rs_type)

df_q_tb$Group2 = paste0(substrLeft(df_q_tb$measure2,1),"_",substrLeft(df_q_tb$measure1,1))
df_q_tb$Group3 = substrLeft(df_q_tb$measure1,5)
